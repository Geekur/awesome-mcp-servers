...
A personal fork of `punkpeye/awesome-mcp-servers` — a giant, hand-curated list (one big README) of ~2,180 Model Context Protocol servers that people can plug into AI assistants. It contains no software; the fork exists solely so a one-line entry advertising his own OpenGolfAPI MCP server could be submitted back to the original list. 
